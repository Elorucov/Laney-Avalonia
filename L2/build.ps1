...
if ($IsWindows) {
    ...
    echo $btagw1;
    $proc1 = Start-Process -NoNewWindow -Wait -FilePath 'dotnet' -PassThru -ArgumentList "publish --nologo -c Release -r win10-x64 --self-contained false --p:PublishSingleFile=true -p:ServerGarbageCollection=true -p:PublishReadyToRun=true -p:Version=$($btagw1) -p:DefineConstants=WIN$($chstr)";
    $proc1.WaitForExit();
    ...
    echo $btagw3;
    $proc2 = Start-Process -NoNewWindow -Wait -FilePath 'dotnet' -PassThru -ArgumentList "publish --nologo -c Release -r win10-arm64 --self-contained false --p:PublishSingleFile=true -p:ServerGarbageCollection=true -p:PublishReadyToRun=true -p:Version=$($btagw3) -p:DefineConstants=WIN$($chstr)";
    $proc2.WaitForExit();
...
if ($IsLinux) {
    ...
    echo $btagl1;
    Start-Process -NoNewWindow -Wait -FilePath 'dotnet' -ArgumentList "publish --nologo -c Release -r linux-x64 --self-contained true -p:EnableCompressionInSingleFile=true --p:PublishSingleFile=true -p:ServerGarbageCollection=true -p:PublishReadyToRun=true -p:Version=$($btagl1) -p:DefineConstants=LINUX$($chstr)";
