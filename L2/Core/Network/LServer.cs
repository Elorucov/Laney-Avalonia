...
namespace ELOR.Laney.Core.Network {
    public class LServer {
        public static async Task<string> StartAndReturnQueryFromClient(CancellationToken ct) {
            string placeholder = $"<html><head><meta charset=\"UTF-8\"><title>Laney</title><script>setTimeout(()=>window.location.href=`${{window.location.origin}}/?${{window.location.hash.substr(1)}}`,300)</script></head><body>{Localizer.Instance["wait"]}</body></html>";
            string placeholderFinal = $"<html><head><meta charset=\"UTF-8\"><title>Laney</title></head><body>{Localizer.Instance["ls_done"]}</body></html>";
